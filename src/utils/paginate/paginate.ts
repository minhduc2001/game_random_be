...
import ServiceUnavailableException from '@/middlewares/exception/ServiceUnavailableException'
...
export async function paginate<T extends ObjectLiteral>(
    ...
    if (Array.isArray(config.relations)) {
      ...
      const createQueryBuilderRelations = (
        ...
        Object.keys(relations).forEach((relationName) => {
          ...
          queryBuilder.leftJoinAndSelect(`${alias ?? prefix}.${relationName}`, `${alias ?? prefix}_${relationName}`)
  ...
  if (config.sortableColumns.length < 1) {
    logger.error("Missing required 'sortableColumns' config.")
